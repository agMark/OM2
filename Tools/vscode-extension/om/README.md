...
- **Insert Figure from Clipboard...**: same as Insert Figure, but takes the image from the clipboard instead of an existing file — copy/capture an image (e.g. Win+Shift+S), run the command, enter a file name, and it saves `img/<name>.png` and inserts the figure markup in one step. Windows only.
- **Data Vars view** (activity bar → "My Custom Explorer" → "Data Vars"): every docVar (`data-vars="X"` placeholder) with its value in each model; click a value to jump straight to that assignment in `docDefs/DocDef_<model>.mjs`. Right-click a variable → **Find Where Data Var Is Used...** to list every `data-vars="X"` use in `html/` (with the models affected) and jump to it. Pairs with **Insert Data Var...** for the other direction (using a var in a fragment).
