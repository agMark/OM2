...
- **Data Vars view** (activity bar → "My Custom Explorer" → "Data Vars"): every docVar (`data-vars="X"` placeholder) with its value in each model; click a value to jump straight to that assignment in `docDefs/DocDef_<model>.mjs`. Pairs with **Insert Data Var...** for the other direction (using a var in a fragment).
